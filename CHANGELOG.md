...
### Fixed
- Cognitive Map mislabelled clusters as `"lighting"` (or any rule-bound category) when a single matching minority actor was present. `ClassifyDominant` in `HaybaMCPCogMapBuilder.cpp` walked classes in count order but returned on the first rule that matched *any* class — so a cluster of 105 `StaticMeshActor` + 1 `DirectionalLight` returned `"lighting"` because StaticMeshActor matched no rule and the loop fell through to the minority Light class. Reworked the classifier to be count-aware: rules now win only when the matching class represents ≥ 35% of the cluster (or is the sole class), and the fallback sanitised name strips a trailing `Actor` so generic clusters label as e.g. `"staticmesh"` instead of `"staticmeshactor"`. New helper `ClassCounts` returns `(name,count)` pairs. **Requires a plugin recompile** to take effect.
