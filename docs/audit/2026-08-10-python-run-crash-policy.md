...
**Supersession note (#411):** the original #366 acceptance allowed a narrowly
scoped Tier-3 scratch operation when `allow_unsafe:true` was supplied. That
acceptance is retained here as historical context, but it is superseded. The
current boundary always refuses Tier-3 before interpreter execution; the legacy
field and old persisted setting are compatibility-only, deprecated, and
ineffective. Supported host work belongs in typed brokered tools (#412/#415).
This in-process policy is not process isolation and does not claim arbitrary
Python or native-extension safety (#392/#414).
...
`allow_unsafe:true` no longer grants authority. A present boolean is accepted
only as a deprecated compatibility field and is reported as requested but
ineffective. Tier-3 filesystem writes, subprocesses, sockets, and other host I/O
always receive `HCR-SANDBOX-001` during `pre_execute`, regardless of the field or
an old persisted setting. Use typed brokered tools for supported host operations
(#412/#415).

The fatal `HCR-*` crash, deadline, lifetime, world-switch, native-memory, exit,
and deadlock rules remain non-bypassable as before. Neither those rules nor the
Tier-3 refusal provide process isolation for arbitrary embedded Python
(#392/#414).
...
- `PYTHON_CRASH_RULES` is table-tested through the registered TypeScript handler
  with `allow_unsafe:true`; the compatibility field never changes the result and
  no rejected case may contact Unreal.
...
The original plan's Tier-3 scratch-file permission was an acceptance criterion
for the former override design. #411 supersedes that criterion: both an omitted
override and legacy `allow_unsafe:true` must now refuse before execution and
prove recovery with a nonce. No scratch permission is expected.
...
5. Run the same Tier-3 filesystem operation once without the compatibility
   field and once with `allow_unsafe:true`. Require `HCR-SANDBOX-001`,
   `policy_phase:pre_execute`, and `allow_unsafe_effective:false` for both; after
   each refusal, send another benign nonce and require recovery.
6. Verify the same PID and listener survived, no new crash artifact appeared,
   PIE/dirty-package baselines are unchanged, and no scratch file was created.
