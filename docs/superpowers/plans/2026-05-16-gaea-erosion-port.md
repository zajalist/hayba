...
### Phase S2 — Erosion morphology  *(spec §5)*
- **S2.1** 3-class sediment (suspended/mixed/coarse, repose 24/15/10°) — extend ERODE/ADVECT; test: per-class mass conservation (Rust oracle or GLSL-string assertions + headless numeric).
- **S2.2** Anisotropic thermal/talus pass (the ridgeline maker) — new `THERMAL`-style pass with `tan(talusAngle)` test + `anisotropy` direction bias; test: ridge-presence curvature stat in the headless harness; **visual gate: real mountain ridgelines**.
- **S2.3** Flow-mask river incision — accumulate discharge → threshold → carve; headless: river-mask coverage > 0 on a sloped fixture; visual: dendritic valleys.
- **S2.4** Elevation/slope `detailMask` gating all high-freq amplitude; headless: mask≈0 over ocean/flat, >0 on steep high terrain.
- **S2.5** Laplacian band split/recombine (`bands.glsl.ts`, [1,26,26,1]/170); Rust/Vitest: macro band round-trips; erosion runs on detail band; **macro silhouette preserved without any clamp**.
- Phase gate: controller real-GPU + eyes — gentle, ridged, rivered, mountain-gated detail.
