import { registerTool, type ToolHandler } from "./index.js";

export const getGraphStateHandler: ToolHandler = async (_args, session) => {
  const state = await session.enqueue(() => session.client.getGraphState());

  const nodeLines = state.nodes.map(
    (n) =>
      `  [${n.cookStatus}] ${n.id} (${n.type})` +
      (Object.keys(n.params).length
        ? `\n    params: ${JSON.stringify(n.params)}`
        : "")
  );

  const edgeLines = state.edges.map(
    (e) => `  ${e.from}:${e.fromPort} → ${e.to}:${e.toPort}`
  );

  const text = [
    `## Current Graph State`,
    ``,
    `### Nodes (${state.nodes.length})`,
    ...nodeLines,
    ``,
    `### Connections (${state.edges.length})`,
    ...(edgeLines.length ? edgeLines : ["  (no connections)"]),
    ``,
    `Call get_parameters(node_id) to inspect a specific node's parameters.`
  ].join("\n");

  return { content: [{ type: "text", text }] };
};

registerTool(
  {
    name: "get_graph_state",
    description:
      "Returns the current Gaea graph — all nodes, their cook status, current parameter values, and connections. Call this to inspect what has been built before deciding what to change.",
    inputSchema: { type: "object", properties: {} }
  },
  getGraphStateHandler
);
