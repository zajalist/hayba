...
export const api = {
  ...
  zones: {
    ...
    submit: (body: { projectId: string; zones: import('./types').Zone[]; masks: { zoneId: string; pngBase64: string }[]; canvasSize?: number; phase?: string }) =>
  ...
  encyclopedia: {
    ...
    addEntry: (projectId: string, entry: import('./types').EncyclopediaEntry) =>
    ...
    getTemplates: (): Promise<import('./types').EncyclopediaEntry[]> =>
