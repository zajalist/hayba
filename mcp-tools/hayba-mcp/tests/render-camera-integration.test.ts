...
describe('render-camera integration', () => {
  ...
  it('unwraps a successful render', async () => {
    ...
    const res = await handleRenderCamera({ camera: { kind: 'transform', location: [0, 0, 0], rotation: [0, 0, 0] } , force: true } as never);
  ...
  it('preserves wait_timeout error', async () => {
    ...
    const res = await handleRenderCamera({ camera: { kind: 'actor', actor: '/Game/X' }, wait_timeout_s: 5 , force: true } as never);
    const body = JSON.parse(res.content[0].text);
  ...
  it('preserves actor_not_found error', async () => {
    ...
    const res = await handleRenderCamera({ camera: { kind: 'actor', actor: '/Game/Missing' } , force: true } as never);
    const body = JSON.parse(res.content[0].text);
