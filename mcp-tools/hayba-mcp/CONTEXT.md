...
- **ToolExecutor** — `src/tools/tool-executor.ts` (planned). The deep seam between Handlers and the TCP envelope. Owns: `ensureConnected`, send/retry, timeout-from-cost, error normalisation. Surface: `executeCommand(cmd, params, opts?) → Promise<data>` resolving with UE's `data` on success, throwing `UeToolError` on failure.
