// mcp-tools/hayba-mcp/src/recipes/composition/frame_target.ts
//
...
//
// The recipe stays pure: it does NOT resolve an actor by path (that
// would need a UE round-trip). Instead the target's world position is
// an explicit input, `target_location`. The UE Recipes panel fills it
...
export const frameTargetExecutor: RecipeExecutor = async (rawParams) => {
