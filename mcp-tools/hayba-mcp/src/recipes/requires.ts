// Recipe ↔ PLUMB bridge.
//
// A recipe may declare a `requires` block (RecipeRequirement[]). Before its side
// effects commit, the runtime should check that the proposed scene satisfies
// BOTH the recipe's inline requirements AND every PLUMB constraint already bound
// to the affected assets/tags — folded into one Verdict. This is the "this tree
// must have these values when using this recipe" guarantee.
//
...
// boundary (recipe_run / executor) and needs the post-effect scene snapshot —
...
export function requirementsToConstraints(spec: RecipeSpec): Constraint[] {
...
export function validateRecipeRequires(spec: RecipeSpec): ValidationError[] {
