...
// recipe. Recipes loaded from disk look up their executor here at run
// time; a missing kind is a clear "executor not bundled" error.

import type { RecipeExecutor } from './types.js';

export class ExecutorRegistry {
  private readonly byKind = new Map<string, RecipeExecutor>();

  register(kind: string, executor: RecipeExecutor): void {
  ...
  get(kind: string): RecipeExecutor | undefined {
