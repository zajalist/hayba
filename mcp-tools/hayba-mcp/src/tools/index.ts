...
import {
  meta as pieActorListMeta,
  schema as pieActorListSchema,
  pieActorListHandler,
} from './pie/pie-actor-list.js';
import {
  meta as pieActorInspectMeta,
  schema as pieActorInspectSchema,
  pieActorInspectHandler,
} from './pie/pie-actor-inspect.js';
import {
  meta as pieProjectWorldMeta,
  schema as pieProjectWorldSchema,
...
const HANDWRITTEN_STANDARD_DESCRIPTORS: ToolDescriptor[] = [
  ...
  },
  {
    ...
    description:
      'Inspect a material, material function, or material instance: properties, parameters, and the full expression graph. Parameter expressions expose their authored parameter_name, parameter_type, typed default_value, and explicit metadata/default availability. For materials, material_outputs[] distinguishes stored property wires from compiler-active terminals (important when Use Material Attributes retains ignored legacy wires). Each expression input includes its source edge (from_node + from_output), each node reports output_consumed + reachable_from_output, and dead_nodes[] lists every node NOT reachable from a compiler-active material output — i.e. provably dead, safe to delete (no delete-recompile-compare needed).',
    meta: materialGetInfoMeta,
    handler: materialGetInfoHandler,
    cost: 'low',
    returns:
      "{kind, name, uses_material_attributes, expressions:[{id,class,x,y,parameter_name?:string|null,parameter_name_valid?,parameter_type?,parameter_metadata_available?,default_value_available?,default_value?,output_consumed,reachable_from_output,reroute_name?,reroute_kind?,inputs:[{name,index,connected,from_node,from_output}],outputs:[{name:string|null,label,index,connect_by_name}]}], material_outputs:[{property,display_name,property_id,connected,compiler_input_active,connection_compiler_used,from_node?,from_output?,from_output_name?:string|null,from_output_label?}], dead_nodes:[{id,class}], comments} | function:{kind,name,expressions,...} | instance:{kind,name,parent,parameters:[{name,type,value}]}. default_value is number for scalar, [r,g,b,a] for vector/double_vector, boolean for static_switch, {r,g,b,a} for static_component_mask, asset path or null for texture/texture_collection/virtual-texture/parameter-collection, and {asset:string|null,page:number} for font. A known null pointer default has default_value_available=true; unavailable/non-finite metadata is null with false, never fabricated. outputs[] gives the exact connectable name where UE has one; unnamed pins have name=null/connect_by_name=false and must be wired with from_output_index (label is display-only).",
    niche: M,
    schema: {
