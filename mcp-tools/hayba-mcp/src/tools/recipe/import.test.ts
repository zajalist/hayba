// mcp-tools/hayba-mcp/src/tools/recipe/import.test.ts
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { mkdtempSync, writeFileSync, rmSync, existsSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { setupRecipeSystem } from '../../recipes/index.js';
import { recipeImportHandler } from './import.js';
...
describe('hayba_recipe_import', () => {
  ...
  it('imports a local file path and installs it to userDir', async () => {
    ...
    const r = await recipeImportHandler({ source: p }, { loader: sys.loader });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.id).toBe('com.example.cool.thing');
      expect(existsSync(join(userDir, 'com.example.cool.thing.recipe.json'))).toBe(true);
    }
  ...
  it('imports an https URL via fetch', async () => {
    ...
    const r = await recipeImportHandler({ source: 'https://example.com/cool.sliver.json' }, { loader: sys.loader });
    expect(fetchSpy).toHaveBeenCalledOnce();
    expect(r.ok).toBe(true);
  });

  it('rejects sources that are not file paths or https URLs', async () => {
    const r = await recipeImportHandler({ source: 'ftp://example.com/x' }, { loader: sys.loader });
